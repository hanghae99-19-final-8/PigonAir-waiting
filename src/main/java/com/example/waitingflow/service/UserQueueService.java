...
public class UserQueueService {
	...
	public Mono<String> generateToken(final String queue, final Long userId) {
		// 캐시에서 해당 토큰을 찾아 반환
		String cachedToken = tokenCache.get(generateCacheKey(queue, userId));
		if (cachedToken != null) {
			return Mono.just(cachedToken);
		}

		// 캐시에 없으면 새로운 토큰을 생성하고 캐시에 저장
		return generateTokenAndCache(queue, userId);
	}
	private Mono<String> generateTokenAndCache(final String queue, final Long userId) {
		return Mono.fromCallable(() -> {
			...
			// 생성한 토큰을 캐시에 저장
			tokenCache.put(generateCacheKey(queue, userId), hexString.toString());

			return hexString.toString();
		}).subscribeOn(Schedulers.boundedElastic());
	}
	private String generateCacheKey(String queue, Long userId) {
		return queue + "-" + userId;
	}
